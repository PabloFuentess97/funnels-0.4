        ...
        <div class="px-4 py-6 sm:px-0">
            <div class="bg-white shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">Editar Link</h3>
                    <p class="mt-1 text-sm text-gray-600">
                        Modifica la información del link.
                    </p>

                    <form action="{{ route('projects.links.update', [$project, $link]) }}" method="POST" class="mt-6">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mt-5 md:mt-0 md:col-span-2">
                                    <div class="grid grid-cols-6 gap-6">
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            ...
                                            <input type="number" name="click_limit" id="click_limit" min="1" value="{{ old('click_limit', $link->click_limit) }}"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                                required>
                                            @error('click_limit')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            ...
                                            <input type="number" name="position" id="position" min="0" value="{{ old('position', $link->position) }}"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                                required>
                                            @error('position')
                                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
